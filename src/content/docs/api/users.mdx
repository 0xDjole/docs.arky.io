...
```typescript
await sdk.user.updateProfilePhone({
  phoneNumber: '+1234567890',
});
...
**Endpoint:** `PUT /v1/users/confirm/phone-number`

<Tabs client:load>
  <div slot="tab1">

```typescript
await sdk.user.verifyPhoneCode({
  phoneNumber: '+1234567890',
  code: '123456',
});
```
...
```bash
curl -X PUT https://api.arky.io/v1/users/confirm/phone-number \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "phoneNumber": "+1234567890",
...
- **[Core Concepts: Authentication](/core-concepts/#authentication)**
- **[Business API: Team Invitations](/api/business/#team-invitations)**
- **[Guides: Authentication](/guides/auth/)**
